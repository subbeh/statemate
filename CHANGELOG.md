...
### Fixed
- `mate apply` no longer asks for your sudo password on every run when a source carries directory ownership attributes such as `etc#owner-r:root`. Existing directories that already had the requested owner, group and mode were re-created, chmodded and chowned via sudo anyway — so `mate status` reported "Everything is up to date" while `mate apply` still prompted. Directories are now only touched when their mode or ownership actually differs
- A source named both in the top-level `sources:` list and under the active profile's `sources:` is no longer scanned twice. Listing it in both places is a reasonable way to say "always, and explicitly here too", but the profile's entries were appended to the global list without removing repeats, so every file in that source was reported as conflicting with itself — `mate status` printed a "conflicting targets detected" warning naming the same source path on both sides, listed each file twice, and offered the source's scripts twice, while `mate apply` and `mate diff` refused to run at all on the bogus conflict
- `mate forget` now accepts a target path relative to the current directory, like the rest of the commands that take a path. Only a leading `~` was expanded, so `mate forget Library/Application\ Support/agent-manager/state.db` run from home failed with "no tracked files match" while the identical path written `~/Library/...` worked — the difference being invisible in the message, which echoed the path back unchanged. Glob patterns resolve the same way, so `mate forget 'app/state.db*'` now matches the sidecars of a tracked database
- `mate managed` now lists `import` in the ATTRIBUTES column. Every other attribute was shown, so a file such as `kubeconfig#import#encrypted` displayed only `encrypted` — indistinguishable from the attribute having been ignored, when in fact it was parsed and honoured all along
...
- Per-profile variants of the same target are no longer reported as conflicting. Two files such as `settings.json#profile:personal` and `settings.json#profile:work` are never deployed together, but the conflict check ran before profile filtering, so `mate status` warned about them, `mate apply` and `mate diff` refused to run, and `mate check` exited 1. Conflicts are now detected among the files the active profile actually deploys; a target claimed twice within one profile chain — a `#profile:base` variant next to a `#profile:work` one, under a profile that `extends: base` — is still a conflict
- `mate status` and `mate apply --dry-run` now report an empty directory a source declares but the target does not have, marked `+`, instead of printing "Everything is up to date" while the next `mate apply` silently created it. Mirroring a directory in a source is the only way to declare one that just has to exist — `~/.cache/ssh/controlmasters` for SSH ControlMaster sockets, a cache directory a program refuses to create itself — but directories were invisible to status unless they already existed with the wrong mode. Directories that hold files are still not listed separately, since they arrive with those files. The `mate apply` summary counts a directory it created, so it no longer reports "applied 1" where `--dry-run` promised 2. Documented in [docs/attributes.md](docs/attributes.md#empty-directories), including the `.gitkeep` + `ignore:` pairing needed because git cannot store an empty directory
- `mate diff` no longer fails with "is a directory" when a directory is among the pending changes (until now only reachable for a directory whose mode drifted from its `#perm:` attribute); it prints the target and `(directory)` instead
...
- `mate packages status -v` now shows `<unknown>` for a package the manager does not recognise, instead of an empty description that was indistinguishable from a package that simply publishes none (many font casks). This surfaces names that can never be installed — a typo such as `github-cli` for `gh`, or a package that only exists on another platform. Homebrew aliases are reported this way too, since only canonical names are matched: declare `node`, not `npm`
- `mate packages status -v` no longer leaves the whole DESCRIPTION column empty when a declared package does not exist. `brew info` fails outright if any single argument is not a known formula or cask, so one bad name — a Linux-only entry such as `man` or `sudo` in a shared `common:` list, or a typo like `github-cli` for `gh` — blanked the descriptions of every other package. Unresolvable names are now filtered out first, so only their own row is blank. The equivalent `pacman -Qi` failure (which exits non-zero whenever a queried package is not installed) had the same effect and is fixed too
- `mate packages status -v` now shows descriptions for Homebrew packages declared by their tap-qualified name (e.g. `jamf/internal-tap/hermes`). `brew` reports these under the bare formula name, which never matched the declared spelling, so every tap package showed a blank description
